...
\end{equation}
where $\bar{E}_j(k)$ and $\sigma_j(k)$ are the mean and standard deviation of the cumulative error within cluster $\mathcal{C}_j$ at step $k$, and $\lambda$ is a safety coefficient. In our offline experiments (Sec.~\ref{sec:experiments}) a value near $\lambda=1.0$ provided a reasonable trade-off for the Square task, but the optimal setting depends on task specifics, dataset coverage and sensing/dynamics noise. Based on the sensitivity and noise‑injection studies reported here, we recommend initializing $\lambda$ in $[0.5,1.5]$ for deployment and tuning it via small closed‑loop trials (Sec.~\ref{sec:deployment_guidance}).
...
\begin{table}[t]
\centering
\caption{\textbf{\(\lambda\) sensitivity (Square, offline evaluation)}. $\bar{k}$ denotes mean predicted horizon; \texttt{Accuracy} reports offline trajectory completion rate.}
\label{tab:lambda_sensitivity}
\small
\begin{tabular}{c c c c}
\hline
$\lambda$ & $\bar{k}$ & sample range (min--max) & success rate (offline) \\
\hline
0.5 & 6.0 & 5--10 & 73.6\% \\
1.0 & 36.5 & 25--50 & 14.2\% \\
2.0 & 38.5 & 35--40 & 59.8\% \\
3.0 & 42.2 & 36--50 & 100.0\% \\
\hline
\end{tabular}
\vspace{2pt}
\flushleft\footnotesize{Note: offline success rate is measured by replaying expert demonstrations; high offline success does not necessarily imply closed‑loop robustness — see the noise‑sensitivity analysis below.}
\end{table}
...
% ---- noise-sensitivity summary (moderate noise std = 0.02) ----
\begin{table}[t]
\centering
\caption{Noise‑injection proxy (state noise std = 0.02) — mean $|\Delta k|$ and fraction of significant changes. Larger $\lambda$ is more sensitive to small perturbations, indicating increased closed‑loop fragility.}
\label{tab:noise_proxy}
\small
\begin{tabular}{c c c}
\hline
$\lambda$ & mean $|\Delta k|$ (noise=0.02) & fraction $|\Delta k|\ge 1$ \\
\hline
0.5 & 0.00173 & 0.17\% \\
1.0 & 0.01412 & 1.41\% \\
2.0 & 0.01815 & 1.82\% \\
...
\end{tabular}
\vspace{2pt}
\flushleft\footnotesize{Note: values are from the offline proxy (see \texttt{experiments/noise_sensitivity_proxy.py}); $|\Delta k|$ is the mean absolute difference in predicted horizon before/after perturbation.}
\end{table}
