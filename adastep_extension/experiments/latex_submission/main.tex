...
\begin{table}[t]
\centering
\caption{\textbf{\(\lambda\) 敏感性（Square，离线评估）}. $\bar{k}$ 为平均预测步长；Accuracy 为离线轨迹完成率。}
\label{tab:lambda_sensitivity}
\small
\begin{tabular}{c c c c}
\hline
$\lambda$ & $\bar{k}$ & 采样范围 (min--max) & 成功率 (离线) \\
\hline
0.5 & 6.0 & 5--10 & 73.6\% \\
1.0 & 36.5 & 25--50 & 14.2\% \\
2.0 & 38.5 & 35--40 & 59.8\% \\
...
\end{tabular}
\vspace{2pt}
\flushleft\footnotesize{注：离线成功率基于对专家演示的回放；高离线成功率并不一定意味着闭环鲁棒，参见下文的噪声敏感性分析。}
\end{table}
...
\begin{table}[t]
\centering
\caption{Noise‑injection代理（state noise std = 0.02）——平均 $|\Delta k|$ 与显著变化比例。较大 $\lambda$ 对小幅扰动更敏感，显示闭环脆弱性上升。}
\label{tab:noise_proxy}
\small
\begin{tabular}{c c c}
\hline
$\lambda$ & mean $|\Delta k|$ (noise=0.02) & 比例 $|\Delta k|\ge 1$ \\
\hline
0.5 & 0.00173 & 0.17\% \\
1.0 & 0.01412 & 1.41\% \\
2.0 & 0.01815 & 1.82\% \\
...
\end{tabular}
\vspace{2pt}
\flushleft\footnotesize{注：表中数值来自离线代理（参见 experiments/noise_sensitivity_proxy.py）；$|\Delta k|$ 为在加噪声前后预测步长的绝对差的均值。}
\end{table}
