...
class StateClusterAnalyzer:
    ...
    def pareto_analysis(self, states: np.ndarray, action_sequences: np.ndarray,
        ...
        for cid in range(self.num_clusters):
            ...
            k_probe = (k_min + k_max) // 2
            complexities = []
            for i in idx:
                if k_probe < action_sequences.shape[1]:
                    complexities.append(self.calculate_linearity_deviation(action_sequences[i], k_probe))
