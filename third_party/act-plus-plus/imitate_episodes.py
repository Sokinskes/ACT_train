...
# AdaStep adapter is optional — import guarded so upstream behavior is unchanged if the module is absent
try:
...
def make_policy(policy_class, policy_config):
    """Factory for policies with minimal, non‑invasive AdaStep support.

    Backward compatible behaviour:
    - If `policy_class` is 'ACT' and `policy_config.get('use_adastep', False)` is true, attach AdaStepAdapter.
    - Accepts explicit alias 'ACT+AdaStep' or 'ACT_ADASTEP'.
    """
    # normalize aliasing
    normalized = policy_class.upper().replace('+', '_')

    if normalized in ('ACT', 'ACT_ADASTEP', 'ACT+ADASTEP'):
        policy = ACTPolicy(policy_config)
        # attach AdaStep adapter if requested
        wants_adastep = (normalized != 'ACT') and ('ADASTEP' in normalized) or policy_config.get('use_adastep', False)
        if wants_adastep:
            if AdaStepAdapter is None:
                # graceful fallback: warn but do not fail (keeps behavior non-destructive)
                print('Warning: AdaStepAdapter not available — continuing without AdaStep')
            else:
                predictor_path = policy_config.get('predictor_path', None)
                ada = AdaStepAdapter(checkpoint_path=predictor_path)
                # attach as attribute expected by downstream eval/training hooks
                setattr(policy, 'predictor', ada)
    elif policy_class == 'CNNMLP':
