...
# 暴露给外部调用的函数
def inject_wind_force(physics, body_name='vx300s_left/gripper_link', force_vector=[50.0, 0.0, 0.0]):
    try:
        body_id = physics.model.name2id(body_name, 'body')
        physics.data.xfrc_applied[body_id][:3] = force_vector
        print(f"🌪️ [物理系统] 阵风突袭！对 {body_name} 施加了外力 {force_vector}")
    except Exception as e:
        print(f"⚠️ [物理系统] 施加外力失败: {e}")

def clear_forces(physics):
    physics.data.xfrc_applied[:] = 0

def teleport_target_object(physics, joint_name='red_box_joint', offset=[0.05, 0.0, 0.0]):
    try:
        joint_id = physics.model.name2id(joint_name, 'joint')
        physics.data.qpos[physics.model.jnt_qposadr[joint_id] + 0] += offset[0]
        physics.data.qpos[physics.model.jnt_qposadr[joint_id] + 1] += offset[1]
        physics.data.qpos[physics.model.jnt_qposadr[joint_id] + 2] += offset[2]
        physics.forward()
        print(f"🎯 [物理系统] 目标物体 {joint_name} 发生了位移 {offset}！")
    except Exception as e:
        print(f"⚠️ [物理系统] 瞬移物体失败: {e}")
